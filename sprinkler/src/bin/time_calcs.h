/* 
 * File:   time_calcs.h
 * Author: Jongilanga Guma (Modified to carter for seconds)
 *
...
struct tm calc_end(struct tm startEnd, int duration)
{
    ...
    startEnd.tm_sec += duration;
    if (startEnd.tm_sec> 59 ) {
        remainder = (startEnd.tm_sec + 1)%60; // tm_sec is a range of 0-59, I want 1-60 for division
        inc = (startEnd.tm_sec + 1)/60;
        startEnd.tm_sec = remainder - 1; // store it back a range of 0-59
        startEnd.tm_min += inc; // increase hours by the amount of additional hours
    } 
