...
if (isset($_GET['action'])) {
        ...
        header('Location: index.php');
               ...
               ';
                ...
                if ($action == "turnOn"){
                    ...
                    mysql_query("UPDATE manual_start SET pump='$_GET[pumps]' WHERE pinNumber='$pin';");
                    mysql_close();
                    //print_r($_GET);
                    header('Location: index.php');
    ...
    print '<form name="autoEdits" action="index.php" method="get"> ';
    print '<table name="Times" border="1" cellpadding="3">
            ...
            <td><select name="pumps"><option value="0">Pump1</option><option value="1">Pump2</option> </select></td>
            </table>';
    ...
    mysql_query("UPDATE auto_start SET pump ='$_GET[pumps]' WHERE pinNumber='$pin';");
    for($i=0;$i<7;$i++)
    {
...
print '
    ...
    <font face="verdana">
    <p>Main Page <a href="index.php?action=setPassword">Change Password</a></p>
    ';
    $temperature = `cat ../temp/temp.txt`;
    $humidity = `cat ../temp/hum.txt`;
    printf("<h3> The current Temperature is  <a href=\"http://ilikeit.co.za/temp/\"> %.1f </a> degrees celsius </h3><br>", $temperature);
    printf("<h3> The current Humidity is  %d %% </h3>", $humidity);
    ...
    print '<tr><th>GPIO #</th><th>GPIO Description</th><th>Status</th><th>Mode</th><th>Action</th><th>Manual Duration (Add 5 seconds extra)</th><th>Edit Description</th><th> Pumps </th></tr>';
...
print '<td align="center">' . $pinNumber . '</td>
    ...
    <td align="center"><img src="' . $image . '" width="50"><td align="center" valign="middle"><form name="pin' . $pinNumber . 'edit" action="index.php" method="get"><input type="hidden" name="action" value="' . $action1 . '"><input type="hidden" name="pin" value="' . $pinNumber . '"><input type="submit" value="' . $buttonValue1 . '"></form></td></td>
    <td><form name="pin' . $pinNumber . 'edit" action="index.php" method="get"><input type="hidden" name="action" value="' . $action . '"><input type="hidden" name="pin" value="' . $pinNumber . '"><input type="submit" value="' . $buttonValue . '"></td>
    <td><input type="text" name="manualDuration" value="' . $manualDuration . '" method="post"></td><td><select name="pumps"><option value="0">Pump1</option><option value="1">Pump2</option> </select></td></form>
    <td><form name="pin' . $pinNumber . '" action="index.php" method="get"><input type="hidden" name="action" value="edit"><input type="hidden" name="pin" value="' . $pinNumber . '"><input type="submit" value="Edit"></form></td>';
    print '</tr>';
    $currentGPIOCount ++;
}
    print '</table>';
